...
class ApiForge:
    ...
    def gen_table_routes(
        ...
        from forge.api.crud import CrudGenerator
    ...
    def gen_view_routes(self, model_manager: ModelManager) -> None:
        ...
        log.info(f"Generated view routes for {len(model_manager.view_cache)} views")
    ...
    def gen_metadata_routes(self, model_manager: ModelManager) -> None:
        ...
        log.info("Generated metadata routes")
    ...
    def gen_health_routes(self, model_manager: ModelManager) -> None:
        ...
        from forge.api.health import HealthGenerator
        ...
        log.info("Generated health routes")
    ...
    def _generate_function_routes(
        ...
        from forge.common.types import ForgeBaseModel, get_eq_type, ArrayType
        from fastapi import Depends, HTTPException

        # Create function factories to avoid closure issues
        def create_procedure_handler(schema: str, fn_name: str, fn_metadata: FunctionMetadata):
        ...
        def create_function_handler(schema: str, fn_name: str, fn_metadata: FunctionMetadata, is_set: bool, OutputModel):
                    ...
                    # For table and set returning functions
                        ...
                        if not records:
                            if is_set:
                                ...
                                default_values = {
                                    field: None for field in OutputModel.model_fields.keys()
                                }
        ...
        for fn_key, fn_metadata in function_cache.items():
            ...
            if route_type == "proc":
                ...
                procedure_handler = create_procedure_handler(schema, fn_name, fn_metadata)
        ...
        if "TABLE" in return_type:
            try:
                ...
                for column in columns:
                    ...
                    if " " not in column:
                        log.warning(f"Invalid column definition in TABLE type: {column}")
                        continue
...
# todo: Add this as a method in the class...
